...
function getFileIcon(extension: string) {
  switch (extension) {
    ...
    case "ts": case "tsx": case "js": case "jsx": case "py": case "rb": case "go": case "rs":
    case "yml": case "yaml": case "toml": case "env": case "example": case "lock": return FileCodeIcon
    case "sh": case "bash": case "zsh": return TerminalIcon
    case "png": case "jpg": case "jpeg": case "svg": case "gif": case "webp": case "ico": return ImageIcon
    case "csv": case "xlsx": case "xls": return TableIcon
    case "zip": case "tar": case "gz": case "rar": case "7z": return FileArchiveIcon
    default: return FileIcon
  }
}

export default function AgentSoloStoragePage() {
  ...
  const navigateTo = (segments: string[]) => setPath(segments)
      ...
      <div className="flex flex-1 flex-col p-4 lg:p-6">
        <div className="flex items-center justify-between mb-4">
          <nav className="flex items-center gap-1 text-sm min-w-0 overflow-x-auto">
            <button onClick={() => navigateTo([])} className="flex items-center gap-1.5 text-muted-foreground hover:text-foreground transition-colors px-1.5 py-1 rounded-md hover:bg-muted/50 shrink-0">
            ...
            {path.map((segment, i) => (
              <span key={`${segment}-${i}`} className="flex items-center gap-1 shrink-0">
                ...
                <button onClick={() => navigateTo(path.slice(0, i + 1))} className={`px-1.5 py-1 rounded-md transition-colors ${i === path.length - 1 ? "text-foreground font-medium" : "text-muted-foreground hover:text-foreground hover:bg-muted/50"}`}>
                  {segment}
                </button>
              </span>
            ))}
          ...
          <div className="flex items-center gap-2 shrink-0 ml-4">
            <span className="text-xs text-muted-foreground/40 tabular-nums">{!loading && `${entries.length} items`}</span>
          </div>
        </div>
        ...
        {error && !loading && (
          <div className="flex flex-col items-center justify-center py-16 text-muted-foreground/50">
            ...
            <button onClick={() => fetchEntries(path)} className="mt-3 text-xs text-primary hover:underline">Retry</button>
          </div>
        )}
        ...
        {!loading && !error && (
          <div className="flex flex-col">
            {path.length > 0 && (
              <button onClick={() => navigateTo(path.slice(0, -1))} className="flex items-center gap-3 px-3 py-2.5 text-muted-foreground hover:text-foreground hover:bg-muted/30 rounded-lg transition-colors group">
                <FolderIcon className="size-4 text-muted-foreground/50" />
                <span className="text-sm">..</span>
              </button>
            )}
            {folders.map((folder) => (
              <button key={folder.name} onClick={() => navigateTo([...path, folder.name])} className="flex items-center gap-3 px-3 py-2.5 hover:bg-muted/30 rounded-lg transition-colors group text-left">
            ...
            {files.map((file) => {
              ...
              return (
                <div key={file.name} className="flex items-center gap-3 px-3 py-2.5 hover:bg-muted/30 rounded-lg transition-colors cursor-default">
